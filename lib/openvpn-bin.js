...
function getSetArgs(args) {
    ...
    switch (process.platform) {
        case 'win32':
            var arg = util.format(new Array('--management %s %d', '--config %s', '--script-security %d', '--management-query-passwords', '--management-hold', '--register-dns').join(','), args.host, args.port, args.config, args.scriptSecurity);
