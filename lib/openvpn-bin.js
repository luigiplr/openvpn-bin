...
function getSetArgs(args) {
    ...
    switch (process.platform) {
        case 'win32':
            return util.format(new Array('--management %s %d', '--config %s', '--script-security %d', '--management-query-passwords', '--management-hold', '--register-dns').join(','), args.host, args.port, args.config, args.scriptSecurity).split(',');
            break;
        case 'darwin':
            ...
            if (args.up && args.down)
                return util.format(new Array(arg, '--up %s', '--down %s').join(','), args.up, args.down).split(',');
            else
                return arg.split(',');
            break;
        case 'linux':
            var arg = util.format(new Array('--management %s %d', '--config %s', '--script-security %d', '--cd %s', '--daemon', '--management-query-passwords', '--management-hold', '--dev', 'tun0').join(','), args.host, args.port, args.config, args.scriptSecurity, args.cwd);
            if (args.up && args.down)
                return util.format(new Array(arg, '--up %s', '--down %s').join(','), args.up, args.down).split(',');
            else
                return arg.split(',');
